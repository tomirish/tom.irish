...
### Print / PDF
- `@media print` stylesheet: white background everywhere, no sidebar shading, no nav bar, no "Back" button, no crimson top border
- Accent color (`#9b2335`) retained for section headings and bullet markers (looks good in print, adds polish to PDF)
- Single-page layout enforced via existing `generate_pdf_browser.py` margins/scale constants
- Print stylesheet always renders single column — sidebar moves below main content, no grid layout
...
| `index.template.html` | Rewrite — clean semantic HTML, new IDs for new sections, remove JS reference |
| `resume.md` | Add GitHub field, Key Achievements section, grouped skills format |
| `scripts/convert_resume.py` | Update parser: grouped skills, Key Achievements, GitHub field |
| `scripts/validate_resume.py` | Update: recognize Key Achievements as valid section, grouped skill format |
| `scripts/generate_pdf_browser.py` | Verify print stylesheet produces correct single-page PDF |
| `tests/` | Update tests to cover new sections and parsing rules |
