...
def test_valid_resume_passes():
    is_valid, _warnings, errors = validate_resume(VALID_RESUME)
...
def test_missing_professional_summary():
    ...
    is_valid, _warnings, errors = validate_resume(content)
    assert not is_valid
...
def test_missing_location_warns():
    ...
    is_valid, warnings, _errors = validate_resume(content)
